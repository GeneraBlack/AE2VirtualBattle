package de.project.ae2virtualbattle.config;

import net.neoforged.neoforge.common.ModConfigSpec;

public class VirtualBattleConfig {
    ...
    static {
        ...
        builder.comment("General Virtual Battle Settings").push("general");

        BASE_TICK_INTERVAL = builder
                .comment("Interval in world ticks between battle drop generation cycles (20 ticks = 1 second, default 60 = 3 seconds)")
                .defineInRange("baseTickInterval", 60, 1, 72000);

        REQUIRE_AE_ENERGY = builder
                .comment("Whether generating battle drops requires AE energy from the ME Network")
        ...
        TIER_1K_DROPS = builder
                .comment("Number of drops per cycle for 1k Virtual Battle Storage Cell")
                .defineInRange("tier1kDrops", 1, 1, 64);

        TIER_4K_DROPS = builder
                .comment("Number of drops per cycle for 4k Virtual Battle Storage Cell")
                .defineInRange("tier4kDrops", 4, 1, 256);

        TIER_16K_DROPS = builder
                .comment("Number of drops per cycle for 16k Virtual Battle Storage Cell")
                .defineInRange("tier16kDrops", 16, 1, 1024);

        TIER_64K_DROPS = builder
                .comment("Number of drops per cycle for 64k Virtual Battle Storage Cell")
                .defineInRange("tier64kDrops", 64, 1, 4096);

        TIER_256K_DROPS = builder
                .comment("Number of drops per cycle for 256k Virtual Battle Storage Cell")
